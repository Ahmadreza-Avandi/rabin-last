...
export default function DocumentsPage() {
    ...
    const fetchDocuments = async () => {
        try {
            ...
            const response = await fetch(`/api/tenant/documents?${params}`, {
                headers: {
    ...
    const handleDownload = async (documentId: string, filename: string) => {
        try {
            ...
            const response = await fetch(`/api/tenant/documents/${documentId}/download`, {
                headers: {
          'Authorization': token ? `Bearer ${token}` : '',
          'X-Tenant-Key': params?.tenant_key || tenantKey,
                },
    ...
    const handleDelete = async (documentId: string) => {
        ...
        try {
            ...
            const response = await fetch(`/api/tenant/documents/${documentId}`, {
                method: 'DELETE',
                headers: {
          'Authorization': token ? `Bearer ${token}` : '',
          'X-Tenant-Key': params?.tenant_key || tenantKey,
    ...
    const sendDocumentByEmail = async (
        ...
        try {
            ...
            const response = await fetch(`/api/tenant/documents/${documentId}/send-email`, {
                method: 'POST',
                headers: {
          'Authorization': token ? `Bearer ${token}` : '',
          'X-Tenant-Key': params?.tenant_key || tenantKey,

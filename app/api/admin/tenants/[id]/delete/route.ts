import { NextRequest, NextResponse } from 'next/server';
import { getMasterConnection } from '@/lib/master-database';
import { requireAdmin } from '@/lib/admin-auth';

async function handleDeleteTenant(request: NextRequest, admin: any, params: { id: string }) {
  let connection;
  
  try {
    const tenantId = params.id;

    connection = await getMasterConnection();

    // Soft delete
    await connection.query(
      `UPDATE tenants 
       SET is_deleted = true,
           updated_at = NOW()
       WHERE id = ?`,
      [tenantId]
    );

    // ثبت لاگ
    await connection.query(
      `INSERT INTO tenant_activity_logs 
       (tenant_id, activity_type, description, performed_by)
       VALUES (?, 'account_deleted', 'حساب کاربری حذف شد (soft delete)', ?)`,
      [tenantId, admin.email]
    );

    return NextResponse.json({
      success: true,
      message: 'حساب با موفقیت حذف شد'
    });

  } catch (error) {
    console.error('❌ خطا در حذف حساب:', error);
    return NextResponse.json(
      { success: false, message: 'خطای سرور' },
      { status: 500 }
    );
  } finally {
    if (connection) {
      connection.release();
    }
  }
}

export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  return requireAdmin((req, admin) => handleDeleteTenant(req, admin, params))(request);
}

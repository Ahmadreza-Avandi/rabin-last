...
export default function SalesPage() {
  ...
  const getStageColor = (stage: string) => {
    switch (stage) {
      case 'بسته شده - برنده': return 'bg-green-100 text-green-800 border-green-200';
      case 'بسته شده - بازنده': return 'bg-red-100 text-red-800 border-red-200';
      case 'مذاکره': return 'bg-blue-100 text-blue-800 border-blue-200';
      case 'ارسال پیشنهاد': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
  ...
  const handleDeleteSale = async (saleId: string) => {
    ...
    try {
      const response = await fetch(`/api/deals?id=${saleId}`, {
        method: 'DELETE'
      });
  ...
  const totalSalesValue = sales.reduce((sum, sale) => sum + sale.total_value, 0);
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card className="border-secondary/20 hover:border-secondary/40 transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium font-vazir">فروش‌های فعال</CardTitle>
            <Calendar className="h-4 w-4 text-blue-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-600 font-vazir">
              {activeSales.length.toLocaleString('fa-IR')}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-accent/20 hover:border-accent/40 transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium font-vazir">فروش‌های موفق</CardTitle>
            <TrendingUp className="h-4 w-4 text-green-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600 font-vazir">
              {wonSales.length.toLocaleString('fa-IR')}
            </div>
          </CardContent>
        </Card>

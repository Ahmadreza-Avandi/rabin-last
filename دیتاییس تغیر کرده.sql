...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 25, 2025 at 02:13 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `interaction_summary`  AS SELECT `activities`.`id` AS `id`, `activities`.`customer_id` AS `customer_id`, `activities`.`type` AS `type`, `activities`.`title` AS `subject`, `activities`.`description` AS `description`, 'outbound' AS `direction`, `activities`.`created_at` AS `interaction_date`, `activities`.`duration` AS `duration`, `activities`.`outcome` AS `outcome`, 'positive' AS `sentiment`, `activities`.`performed_by` AS `user_id` FROM `activities`union all select `feedback`.`id` AS `id`,`feedback`.`customer_id` AS `customer_id`,`feedback`.`type` AS `type`,`feedback`.`title` AS `subject`,`feedback`.`comment` AS `description`,'inbound' AS `direction`,`feedback`.`created_at` AS `interaction_date`,0 AS `duration`,`feedback`.`status` AS `outcome`,`feedback`.`sentiment` AS `sentiment`,`feedback`.`assigned_to` AS `user_id` from `feedback`  ;

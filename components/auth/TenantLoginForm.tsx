...
export default function TenantLoginForm({ tenantKey, onSuccess }: TenantLoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/tenant/auth/login', {
        ...
        body: JSON.stringify({ email, password, tenant_key: tenantKey }),
      });
      ...
      if (data.success) {
        ...
        if (onSuccess) {
          ...
          setTimeout(() => {
            window.location.href = `/${tenantKey}/dashboard`;
          }, 100);
        }
      } else {
